...
/**
 * The two Drive folders whose creatives we are cleared to use, and the short
 * folder name each maps to in the UI (the panel only fits ~20 characters).
 */
const PRODUCTS = {
...
/** Only two creative sets were approved for use, so every offer maps onto one. */
const productOf = (label) => (/home fitness/i.test(label) ? "home-fitness" : "slimkit-walking");
  ...
  for (const product of Object.keys(PRODUCTS)) {
    ...
    pool[product] = await Promise.all(
      files.map(async (file) => {
        ...
        const [w, h] = video ? mp4Size(buf) : jpegSize(buf);
        return { src: `/creatives/${product}/${file}`, video, size: `${w}x${h}` };
      }),
    );
  }
  return pool;
...
const filled = backfillClicks(offers);
...
for (const offer of offers.sort((a, b) => a.offIdNet.localeCompare(b.offIdNet))) {
  // The offer gets re-tagged mid-flight (cap notes, creative-rotation markers); the
  // longest name is the most recent and most descriptive one.
  const rawName = [...offer.names].sort((a, b) => b.length - a.length)[0];
  const label = offerLabel(rawName);
  const product = productOf(label);
  const days = [...offer.days.entries()].sort(([a], [b]) => a.localeCompare(b));

  const r = rng(hash(offer.offIdNet));
  const between = (lo, hi) => lo + r() * (hi - lo);

  // CPA payout for a US iOS weight-loss install. Fixed per campaign so ROAS moves
  // with conversions rather than with a re-rolled payout.
  const payout = Math.round(between(18, 32) * 100) / 100;
  ...
  const creatives = Array.from({ length: n }, (_, i) => {
    ...
    return {
      ...
      name: `${product.replace(/-/g, " ")} ${index + 1}.${asset.video ? "mp4" : "jpg"}`,
      ...asset,
  ...
  campaigns.push({
    ...
    creatives,
    // [date, clicks, conversions] — the only numbers that come from the sheet.
    days: days.map(([d, [c, e]]) => [d, c, e]),
  });
}

const ts = `/**
 * Campaign roster and real per-day traffic, generated from the shared performance
 * spreadsheet by \`scripts/build-campaigns.mjs\`. Do not edit by hand — re-run the
 * script instead.
 *
 * \`days\` holds \`[date, clicks, conversions]\`. Those two counters are the sheet's
 * \`Total Lead\` and \`Event\` columns; the rest of the funnel is derived in
 * \`demo-data.ts\` so there is one place where a number is invented.
 */
...
export interface Campaign {
  ...
  product: string;
  productLabel: string;
  /** CPA payout per conversion, in dollars. */
  payout: number;
  ...
  days: [date: string, clicks: number, conversions: number][];
}
